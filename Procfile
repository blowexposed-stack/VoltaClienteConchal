...
from database.models import (
    ...
    buscar_config_retorno, salvar_config_retorno,
    buscar_clientes_para_lembrete_hoje,
)
...
def dashboard():
    ...
    horarios     = listar_horarios(usuario["id"])
    agend_pub    = listar_agendamentos_publicos(usuario["id"])
    config       = buscar_config_retorno(usuario["id"])
    return render_template(
        "dashboard.html",
        usuario=usuario,
        clientes=clientes,
...
def salvar_cliente():
    ...
    dias_retorno = request.form.get("dias_retorno", "7").strip()
    ...
    try:
        dias_retorno = max(1, int(dias_retorno))
    except ValueError:
        dias_retorno = 7

    criar_cliente(usuario["id"], nome, celular, servico, dias_retorno)
    from datetime import timedelta
    data_lembrete = (datetime.now() + timedelta(days=dias_retorno)).strftime("%d/%m/%Y")
    flash(
        f"Cliente {nome} cadastrado. Lembrete automático gerado pela IA em {dias_retorno} dias ({data_lembrete}).",
        "sucesso"
    )
    return redirect(url_for("dashboard"))


@app.route("/salvar_config_retorno", methods=["POST"])
@requer_acesso
def rota_salvar_config_retorno():
    usuario     = usuario_logado()
    dias_padrao = request.form.get("dias_padrao", "7").strip()
    try:
        dias_padrao = max(1, int(dias_padrao))
    except ValueError:
        dias_padrao = 7
    salvar_config_retorno(usuario["id"], "", dias_padrao)
    flash(f"Prazo padrão atualizado para {dias_padrao} dias.", "sucesso")
    return redirect(url_for("dashboard"))
